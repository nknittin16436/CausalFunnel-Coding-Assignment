...
export class BlogController {
    ...
    @UseGuards(AuthGuard)
    @Post('')
    createBlog(@Body() createBlogData: CreateBlog): Promise<BlogSuccess> {
        return this.blogService.createBlog(createBlogData);
    }

    @UseGuards(AuthGuard)
    @Patch('/:id')
    updateBlog(@Body() updateBlogData: UpdateBlog, @Param('id') id: string): Promise<BlogSuccess> {
        return this.blogService.updateBlog(id, updateBlogData);
    }

    @UseGuards(AuthGuard)
    @Delete('/:id')
    deleteUser(@Param('id') id: string): Promise<BlogSuccess> {
